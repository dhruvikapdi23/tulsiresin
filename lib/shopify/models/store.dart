...
class Store {
  ...
  Store.fromDokanJson(Map<String, dynamic> parsedJson) {
    ...
    if (parsedJson['rating'] != null) {
      if (parsedJson['rating']['rating'] != null && double.tryParse(parsedJson['rating']['rating']) != null) {
    ...
    address = parsedJson['address'] is Map ? parsedJson['address']['street_1'] : '';
    banner = parsedJson['banner'] is String && isNotBlank(parsedJson['banner']) ? parsedJson['banner'] : null;
    ...
    try {
      if (parsedJson['social'] is Map) {
        ...
        String? key = social.keys.firstWhere((o) => isNotBlank(social[o]), orElse: () => '');
        if (isNotBlank(key)) {
          website = social[key];
        }
      }
  ...
  Store.fromWCFMJson(Map<String, dynamic> parsedJson) {
    ...
    rating = double.parse((parsedJson['store_rating'] != null && parsedJson['store_rating'].toString().isNotEmpty) ? parsedJson['store_rating'].toString() : '0.0');
    address = parsedJson['vendor_address'];

    if (parsedJson['settings'] != null && parsedJson['settings'] is Map) {
      image = parsedJson['settings']['gravatar'].toString().isNotEmpty && parsedJson['settings']['gravatar'].toString().contains('http') ? parsedJson['settings']['gravatar'] : null;
      banner = parsedJson['settings']['mobile_banner'].toString().isNotEmpty && parsedJson['settings']['mobile_banner'].toString().contains('http') ? parsedJson['settings']['mobile_banner'] : null;
      if (isBlank(banner)) {
        banner = parsedJson['settings']['banner'].toString().isNotEmpty && parsedJson['settings']['banner'].toString().contains('http') ? parsedJson['settings']['banner'] : null;
      }
      if (isBlank(banner)) {
        banner = parsedJson['mobile_banner'] ?? parsedJson['vendor_banner'] ?? parsedJson['vendor_list_banner'];
      }
      ...
      if (parsedJson['settings']['wcfm_store_hours'] != null) {
        storeHour = StoreHour.fromWCFM(parsedJson['settings']['wcfm_store_hours']);
      }
      vacationSettings = VacationSettings.fromJson(parsedJson['settings']);
    } else {
      banner = parsedJson['mobile_banner'] ?? parsedJson['vendor_shop_logo'] ?? parsedJson['vendor_banner'] ?? parsedJson['vendor_list_banner'];
    }
  }

  Map<String, dynamic> toJson() {
    return {'id': id, 'name': name, 'email': email, 'rating': rating, 'image': image, 'address': address, 'banner': banner, 'phone': phone, 'website': website};
  }
...
class WorkingHour {
  ...
  WorkingHour.fromWCFM(String startTime, String endTime) {
    final now = DateTime.now();
    start = DateTime.parse('${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')} $startTime:00');
    end = DateTime.parse('${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')} $endTime:00');
  }
