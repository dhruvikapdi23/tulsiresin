...
class Cart extends StatelessWidget {
            ...
            children: [
              cartCtrl.productsInCart.isNotEmpty
                  ? Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...createShoppingCartRows(context),
                        ...
                        Stack(
                          ...
                          children: [
                            SizedBox(
                                ...
                                child: Text(
                                  CommonFonts().youBagIsEmpty,
                                  textAlign: TextAlign.center,
                                ).fontSize(FontSizes.s18).fontWeight(FontWeight.w600).width(MediaQuery.of(context).size.width)),
                            SizedBox(
                                width: MediaQuery.of(context).size.width,
                                child: Image.asset(
              ...
              CustomButton(
                ...
                radius: 0,
                style: TextStyle(fontWeight: cartCtrl.cartList.isNotEmpty ? FontWeight.normal : FontWeight.w700, color: appCtrl.appTheme.surface),
              ),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [Text(CommonFonts().myWishList).fontSize(FontSizes.s18).textColor(appCtrl.appTheme.black), Text(CommonFonts().showAll).fontSize(FontSizes.s14).textColor(appCtrl.appTheme.primary).gestures(onTap: () => Get.toNamed(routeName.product))],
  ...
  List<Widget> createShoppingCartRows(BuildContext context) {
    printLog("::: createShoppingCartRows");
    return cartCtrl.productsInCart.keys.map((key) {
      var productId = cartCtrl.cleanProductID(key);
      var product = cartCtrl.getProductById(productId);

      if (product != null) {
        return CartListCard(
          product: product,
          quantity: cartCtrl.productsInCart[key],
          optionLength: product.options!.length,
          minusTap: () {
            int qunatity = product.quantity!;
            if (qunatity == 1) {
              qunatity = 1;
            } else {
              qunatity--;
          ...
          onTap: () => Get.toNamed(routeName.productDetail, arguments: product),
        );
      }
